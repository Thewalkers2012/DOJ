...
void init_syscalls_limits(int lang) {
  ...
  if (record_call) {  // C & C++
    ...
    for (i = 0; LANG_JC[i]; i++) call_counter[LANG_JV[i]] = LANG_JC[i];
  }
}
...
void run_solution(char *infile, int &usedtime) {
  ...
  if (lang == LangC || lang == LangCC) {
    ...
    execl("/usr/bin/java", "/usr/bin/java", "-Xms128M", "-Xms512M",
          "-Djava.security.manager", "-Djava.security.policy=./java.policy",
          "-DONLINE_JUDGE=true", "Main", (char *)NULL);
  }
...
//评判用户 solution 的结果
void judge_solution(int &judge_flag, int usedtime, int topmemory, char *outfile,
                    char *userfile) {
...
void watch_solution(pid_t pidApp, char *userfile, char *outfile,
                    int &judge_flag, int &topmemory, int &usedtime) {
  // parent
  int tempmemory;
  ...
  while (1) {
    ...
    if (lang == LangJava) {
      ...
      write_log("VmPeak %d, VmData %d", get_proc_status(pidApp, "VmPeak:"),
    ...
    if (!record_call &&
        call_counter[reg.REG_SYSCALL] ==
            0) {  // do not limit JVM syscall for using different JVM
      judge_flag = JudgeRE;
...
void init_parameters(int argc, char **argv) {
  if (argc < 4) {
    fprintf(stderr,
            "Usage:%s [language] [time limit] [memory limit] [run path]\n",
            argv[0]);
    exit(1);
  }
  ...
  if (lang == LangJava) {
    ...
    time_lmt = time_lmt * 2;
    mem_lmt = mem_lmt * 2;
    execute_cmd("cp /etc/java-7-openjdk/security/java.policy %s/java.policy",
                work_dir);
  }
...
int main(int argc, char **argv) {
  ...
  for (; (judge_flag == JudgeAC) && (dirp = readdir(dp)) != NULL;) {
    ...
    if (pidApp == 0) {
      ...
      watch_solution(pidApp, userfile, outfile, judge_flag, topmemory,
                     usedtime);
  ...
  write_log("result = %d usedtime = %d topmemory = %d", judge_flag, usedtime,
            topmemory);
